...
Capricorn::Capricorn(Map* owner, Vec2 const& startPos, float orientation)
	: Entity(owner, startPos, orientation)
{
	...
	m_shootDelay = g_gameConfigBlackboard.GetValue("capricornFireDelay", 0.f);
	...
	m_physicsRadius = g_gameConfigBlackboard.GetValue("capricornPhysicsRadius", 0.f);
	m_cosmeticRadius = g_gameConfigBlackboard.GetValue("capricornCosmeticRadius", 0.f);
	m_bodyTexture = g_theRenderer->CreateOrGetTextureFromFile("Data/Images/EnemyTank3.png");
}
...
void Capricorn::UpdateTransform(float deltaSeconds)
{
	...
	if (m_isPursuing) // pursuing
	{
		...
		if (targetShootDegrees < fireHalfAperture && m_hasSightOfPlayer)
		{
			...
			if (m_shootTimer >= m_shootDelay)
			{
				...
				Entity* newBullet = m_map->CreateEntityOfType(ENTITY_TYPE_EVIL_MISSLE, spawnPos, m_orientationDegrees);
...
void Capricorn::ReactToBulletHit(Bullet& bullet)
{
...
void Capricorn::RenderEntity() const
{
